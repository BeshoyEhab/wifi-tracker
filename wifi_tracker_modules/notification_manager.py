...
class NotificationManager:
    ...
    def send_notification(self, title: str, message: str, urgency: Urgency = Urgency.NORMAL) -> bool:
        ...
        try:
            subprocess.run(
                ['notify-send', '--app-name=WiFi Tracker', f'--urgency={urgency.value}', title, message],
    ...
    def _ask_with_notify_send_sh(self, title: str, body: str, actions: dict, timeout: int = 60) -> str:
        """
        ...
        while time.time() - start < timeout:
            try:
                cmd = [
                    ...
                    '--radiolist',
                    '--ok-label=Apply', '--cancel-label=Ignore',
    ...
    def ask_gateway_trust(self, ssid: str, gateway_ip: str, mac: str = "", vendor: str = "") -> str:
        ...
        if self.notify_send_sh:
            ...
            choice = self._ask_with_notify_send_sh("Unknown Gateway Detected", body, actions, timeout=60)
            if choice == "Trust":
                return "trust"
            elif choice == "Block":
                return "block"
    ...
    def ask_high_usage_action(self, ssid: str, app_name: str, size: str, window: str) -> str:
        ...
        if self.notify_send_sh:
            ...
            choice = self._ask_with_notify_send_sh("High Data Usage Alert", body, actions, timeout=60)
            mapping = {
                "Safe once": "safe_once",
